...
>
> Two apps run the same underlying model, yet one is helpful and casual while the other is formal and restrictive. If the engine is identical, where does the difference in behavior live?

<details>
<summary>Hint</summary>

Think about everything that reaches the model before and alongside your message.
...
An AI product is never a bare model. Every message you send arrives at the model wrapped in a **harness**: a **system prompt** it read before it ever saw you, a set of tools it may call, files or knowledge it can search, and a memory layer it consults. The engine supplies capability. The car decides what that capability feels like on the road: a race car, a delivery van, and a school bus can share an engine and remain completely different vehicles.
...
> **Interactive**
>
> This unit has an interactive exercise (`harness-inspector`) that runs in the ThinkModel reader at https://thinkmodel.ai. It cannot be reproduced in plain text.
...
> **Case study: The harness becomes a product category**
>
...
> **Watch out: Common misconception: a packaged assistant is neutral**
>
...
> **Think about this before reading on**
>
> A friend shares a custom bot that gives "brutally honest feedback." Before pasting your essay into it, what would an operator want to know about its harness?

<details>
<summary>Hint</summary>

Instructions, tools, data, author, incentive.

</details>

**Knowledge check —** The same model behaves like a cheerful tutor in one app and a strict compliance checker in another. The best explanation, and the operator's takeaway, is:
...
Same engine, different car. The wrapping (what it was told, what it can touch, what it remembers) is doing the work, which is excellent news: the wrapping is exactly the part you can see, question, and build. Chasing engines is consumer behavior; designing harnesses is operating.
...
>
> Uses whatever behavior each app ships. Re-explains personal workflows every session. Judges products by engine gossip ("which model is it?") and inherits every hidden author's goals without noticing.

> **Building and inspecting harnesses**
>
> Reads any AI product as engine plus wrapping, probes the wrapping in one prompt, and packages personal workflows as skills that load on demand. Owns a growing library that survives every engine swap.
...
> **Think about this before reading on**
>
> Models get replaced under you every few months. What, concretely, do you get to keep across those replacements?

<details>
<summary>Hint</summary>

Inventory what you've made in Units 01 through 05.

</details>

**Knowledge check —** Rami has explained "how to summarize my lecture notes" to his assistant four times this month. The operator's move is to:
...
**harness** — Everything a product wraps around an AI model: the system instructions, the tools it may use, the files and knowledge in reach, and the memory it keeps. The harness, more than the model, determines behavior.

**system prompt** — The standing instructions a product gives the model before any user message arrives. You usually can't see it, but it shapes tone, rules, and refusals more than anything you type.
