...
:::socratic
question: A research agent reads three hundred pages for you and writes a confident summary. What exactly did you delegate, and what's still yours?
hint: Separate the activity (finding, reading, compiling) from the outcome (believing a claim enough to act on it).
concept: You can delegate the reading. You cannot delegate the believing, because belief carries the responsibility for what you do next, and the agent doesn't bear consequences.
:::
...
:::analogy-target{id="p-research-relay"}
...
## The report that looked like diligence
...
## Common misconception: many sources mean strong evidence
...
:::socratic
question: An agent's report cites forty sources. Your decision depends on three claims in it. What's the highest-value twenty minutes of verification you can do?
hint: You can't check everything, and you don't need to. What does the decision rest on?
concept: Verification is triage. Identify the load-bearing claims and chase each to its primary source. Everything else in the report is scenery.
:::

:::quiz{id="c2u03-q1" objective="U03-O1"}
question: Two students research the same purchase decision. Amal types "best budget laptops" into a research agent. Zain gives it: his budget, his three uses, a request to prefer manufacturer specs and independent reviews over affiliate listicles, a comparison-table output shape, and an instruction to flag anything uncertain. Why will Zain's report be dramatically more useful?
correct: D
...
feedback_correct: A research agent makes hundreds of small decisions while it works: which searches to run, which links to open, what to trust, what to include. A brief steers every one of those decisions; a bare question leaves every one to a guess. Length isn't the mechanism. Steering is. This is Unit 01's lesson applied to a mission instead of a workspace: the machine that writes the brief now writes it for an agent.
feedback_incorrect: The answer is D. Word count and search time aren't the mechanism; steering is. An agent working autonomously for ten minutes makes hundreds of judgment calls, and the brief is your only way to be present for them. Scope, source preferences, output shape, and uncertainty flags convert a generic tour into your investigation.
:::
...
:::compare
before_label: The report reader
before: Asks a bare question, receives forty formatted pages, reads the summary, acts on it. Feels thorough because the report looks thorough. Has outsourced breadth, depth, and belief in one click, and will discover the weak claim at the worst possible time.
after_label: The relay operator
after: Commissions with a brief, hands the report into a grounded notebook with its key sources, interrogates it against the real decision, and personally verifies the three claims that carry weight. Ten extra minutes. The belief stays home.
:::
...
:::socratic
question: As research agents get better, their errors get rarer but harder to spot. Does that make your verification pass more important or less?
hint: Think about what "rarer but better camouflaged" does to the cost of the errors that remain.
concept: Better agents concentrate risk into fewer, more convincing failures. Verification stays load-bearing; it just moves from catching many obvious errors to catching the rare confident one.
:::

:::quiz{id="c2u03-q2" objective="U03-O2"}
question: A research agent's report says a supplement improves sleep "according to multiple studies," citing four sources. Before acting on it, the highest-value check is:
correct: B
...
feedback_correct: This is delegating the reading, never the believing, in one move. Four citations can be four echoes of one press release; a source can be real and still not say what the report says it says. Opening the sources checks independence and fidelity at once, and it's a ten-minute job because you're only doing it for the claim you'll act on. Cross-checking in a second tool (option D) has some value, but both tools read the same internet, so agreement can just mean they found the same original error.
feedback_incorrect: The answer is B. Citation count measures gathering effort, not truth: echoes of one original inflate it. Self-confirmation produces more generated text, not evidence. And a second tool searching the same internet can converge on the same error. Only opening the sources tests the two things that matter: independence, and whether they say what the report claims they say.
:::

## The Challenge

:::challenge{title="The Verified Research Relay" time="45 minutes"}
