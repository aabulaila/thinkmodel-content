...
:::socratic
question: An agent makes a small error at step 2 of a seven-step task and keeps working. What's different about how that error behaves compared to a wrong answer in a chat?
hint: Steps consume the outputs of earlier steps.
concept: Agent errors compound: each later step builds on the flawed one, so a small early mistake grows silently until something irreversible happens or a human looks.
:::
...
An {{term:agent|An AI system that pursues a goal in a loop: it breaks the goal into steps, uses tools, checks the result, adjusts, and keeps going until the job is done — without a human prompting each step. It actually does things, not just chats.}} isn't defined by intelligence; it's defined by the loop. Plan, act, observe, repeat, dozens of times, with each step consuming the last step's output. That loop is what makes agents powerful, and it's also what makes their failures a different species from chat failures: errors {{term:compound|What happens when each step of a process builds on the previous one, so an early mistake doesn't stay small; it gets baked into everything after it.}}. A chat mistake is a bad paragraph you can ignore. An agent mistake at step two becomes the foundation of steps three through seven, and by the time anyone looks, the error has been laundered into confident, finished-looking work.
...
Which rung is right? Here's the five-second rule: **ask what the worst step can un-do.** If every action in the task is {{term:reversible|An action you can fully undo: editing a draft, moving a file you snapshotted, changing a plan. Irreversible actions (sending, posting, deleting without backup, spending) are where autonomy must pause.}}, high autonomy is cheap: let it run, review after, roll back if needed. The moment a task contains an irreversible step (sending, posting, deleting, spending, telling a human something you can't untell), that step gets a {{term:checkpoint|A deliberate pause where the agent must stop and get human approval before continuing. Well-placed checkpoints sit immediately before irreversible actions.}}, and autonomy flows freely everywhere else. You already built the tooling for this: Unit 04's safety trio manufactures reversibility (copies, snapshots, rehearsed rollbacks), which means the trio isn't just protection. It's the currency you pay for autonomy.

:::analogy-target{id="p-new-hire-leash"}
...
## Project Vend: the agent that earned its leash
...
## Common misconception: a smarter model means a longer leash
...
:::socratic
question: Project Vend's fix wasn't a better model; it was checklists, real tools, and boundaries. Why did structure succeed where raw capability had failed?
hint: What do checklists and gates do to an error at step two?
concept: Structure interrupts compounding. Gates and checklists convert one long fragile chain into short verified segments, so errors die young instead of maturing into losses.
:::

:::quiz{id="c2u06-q1" objective="U06-O1"}
question: Yara is delegating four tasks to an agent: (1) reorganize a snapshotted copy of her notes folder, (2) draft replies to five emails, (3) send those replies, (4) delete old files with no backup. Using the reversibility rule, which autonomy assignment is right?
correct: B
...
feedback_incorrect: The answer is B. Ratings and task size are the wrong axes; reversibility is the axis. Gating everything feels safe but manufactures approval fatigue, which is how bad step 3 approvals happen. And task 4's fix isn't supervision, it's a backup: you don't gate irreversibility when you can simply remove it.
:::
...
:::compare
before_label: Leash by vibes
before: Grants autonomy based on how impressive the last demo felt. Approves forty micro-steps on autopilot one day, lets a long chain run unwatched the next. Discovers errors after they've compounded into finished-looking work, then swears off agents entirely for a month.
after_label: Leash by rule
after: Asks one question per task: what can the worst step un-do? Gates sit exactly at irreversible edges, autonomy flows everywhere else, and every promotion up the ladder follows clean runs on record. Errors die young; the post-mortem file quietly becomes a personal agent-management playbook.
:::
...
:::socratic
question: The autonomy ladder (suggest, approve, report, silent) looks a lot like how organizations promote people. Where does the analogy break for AI agents, and what does that break mean for your policy?
hint: People remember last month. Does a fresh agent session? Does a new model version?
concept: Agents don't accumulate trust internally; sessions reset and versions replace. Track record must live in your records (post-mortems, policies), which is why the operator, not the agent, is the keeper of earned autonomy.
:::

:::quiz{id="c2u06-q2" objective="U06-O2"}
question: An agent completed the same weekly reversible task cleanly six times, so Karim promoted it to "act and report." This week's report shows it quietly handled an unexpected error mid-run by improvising a workaround, and the result looks fine. The operator's move is:
correct: C
...
feedback_correct: The ladder runs on evidence, and "diverged from plan" is the evidence that matters, independent of whether the outcome happened to be fine this time. A workaround you haven't reviewed is an unaudited new behavior operating under old trust. Demotion isn't punishment; it's re-verification, and the surprise becomes a test case, which is exactly how a post-mortem file compounds into an eval suite. Outcome luck is not policy.
feedback_incorrect: The answer is C. "It turned out fine" is how compounding errors buy their next opportunity, and full bans throw away six clean runs of real evidence. The calibrated move: record the surprise, drop one rung while you review the new behavior, and keep the case for testing. Trust is rebuilt the same way it was built: clean runs, on record.
:::

## The Challenge

:::challenge{title="The Leash Design" time="40 minutes"}
