...
:::embed{type="video" title="It Doesn't Know Anything" src="/api/storage/public-objects/videos/4-The_Explainer_-_It_Doesn_t_Know_Anything.mp4" youtubeId="P6s1rIkC8MY"}
:::

:::socratic
question: The "strawberry" mistake got fixed — models now count those R's correctly. Does that mean AI finally learned to see letters?
hint: Think about the difference between changing what the system reads as input and teaching it a workaround, like spelling a word out before counting it.
concept: The input never changed — the model still reads text as multi-letter chunks (tokens), not letters. Newer models cope by spelling the word out step by step before counting. The limitation was worked around, not removed.
:::

## The Core Concept

Here's what's actually happening when you talk to an AI like Claude or ChatGPT. You type a message. The system converts your words into numbers — little chunks called {{term:tokens|Small chunks of text (roughly ¾ of a word) that AI reads and writes one at a time. AI sees tokens instead of individual letters — the root cause of famous early failures like miscounting the R's in "strawberry" — and usage is priced per million of them, which is why long tasks cost more.}}. Then it does one thing, over and over: {{term:next-token prediction|The way AI generates all of its text: it looks at everything so far, guesses the most likely next chunk, and repeats one piece at a time. Uploads and memories improve the input; they don't change the engine.}}.
...
:::analogy-target{id="p-autocomplete"}
...
## Mata v. Avianca: The case that put AI hallucination on trial
...
This is also why AI {{term:hallucination|When AI makes up information that sounds true but isn't, like fake facts or made-up sources. It's guessing what sounds right, not checking if it's true.}}s — a term for when it generates information that sounds true but isn't. The AI has no concept of truth. It was doing what it always does: predicting plausible-sounding text based on the patterns it learned. And a fake case citation looks, statistically, a lot like a real one.

Researchers Emily Bender and Timnit Gebru coined a useful metaphor: they called {{term:large language models|AI systems trained on massive amounts of text that can read, write, and have conversations. ChatGPT and Claude are examples.}} {{term:stochastic parrots|A way of describing how AI chatbots work. They repeat text that sounds meaningful based on patterns, but don't actually understand any of it, like a parrot mimicking words without knowing what they mean.}}. A parrot can repeat words that sound meaningful to you, but the parrot doesn't understand what the words mean. It's producing sounds that match patterns it was exposed to. Language models do something similar at a much larger scale — they produce text that matches patterns in training data. Whether that qualifies as "understanding" is genuinely contested today, especially for newer reasoning-trained models that visibly work through problems step by step. But the practical point still holds: the system has no built-in commitment to truth, and fluent text is not evidence of accurate text.

:::callout{type="deep-dive"}
## Why the "stochastic parrot" metaphor matters
...
:::socratic
question: The lawyer in the Mata v. Avianca case asked ChatGPT to confirm that its fake citations were real, and it said yes. Why would AI confirm something that isn't true?
hint: Think about what "confirming" means for a system that predicts plausible-sounding text. Does it check facts, or does it predict what a confirmation typically sounds like?
concept: AI doesn't verify claims — it generates text that matches the pattern of a confirmation. Asking AI to fact-check itself is unreliable because it's still just predicting plausible responses, not checking truth.
:::
...
:::quiz{id="u04-q1"}
question: AI writes a convincing essay about quantum physics but gets basic arithmetic wrong. The best explanation is:
correct: B
...
feedback_correct: Exactly. The AI isn't "doing physics" or "doing math" — it's predicting the next token based on patterns. There are millions of physics explanations in its training data, giving it strong patterns to match. But when asked 7 × 8, it's still predicting text, not computing. When the pattern doesn't match the correct answer, you get a confident wrong result.
feedback_incorrect: The answer is B. The underlying language model doesn't "understand" any subject — everything it does is next-token prediction. It produces good physics essays because there are many similar texts in training data. Predicting what text usually follows "7 × 8 =" isn't the same as multiplying, which is why modern AI products quietly route math through a calculator or a step-by-step reasoning process. The language model itself is still pattern-matching, not calculating.
:::
...
:::embed{type="tool" title="OpenAI Tokenizer — See how AI breaks text into tokens" url="https://platform.openai.com/tokenizer"}
:::
...
:::compare
before_label: What it feels like
before: AI gives confident, fluent, well-structured answers that feel like expertise. It uses "I think" and "I believe." It cites sources. It sounds like it understands the topic deeply and has thought carefully about its response.
after_label: What's actually happening
after: At its core, the AI is predicting the most likely next chunk of text based on patterns in training data. "I think" is pattern-matched phrasing, not introspection. Citations may be fabricated. And critically: the system sounds equally confident whether it's right or wrong — fluency is not evidence of accuracy.
:::
...
:::socratic
question: People naturally say AI "thinks," "knows," and "believes." Why is this tendency dangerous, and how should you think about AI's responses instead?
hint: Think about why the lawyer trusted ChatGPT's fake cases — the language sounded authoritative. What happens when you assume understanding behind fluent text?
concept: Anthropomorphizing AI — treating it as if it understands — leads to over-trust. AI produces pattern-matched text, not informed judgments. Treating it as a draft generator, not an authority, is the key mindset shift.
:::

:::quiz{id="u04-q2"}
question: A student uses AI to research a topic and the output includes a specific statistic with a citation. The right approach is to:
correct: C
...
feedback_correct: This is the core skill. AI-generated text is plausible by default — it's designed to produce text that fits the pattern. That means both the statistic and the citation could be fabricated. The 30 seconds it takes to verify a claim independently is the difference between using AI well and being used by it.
feedback_incorrect: The answer is C. AI confidence has zero correlation with accuracy — it sounds equally confident whether it's right or fabricating. And ignoring AI entirely misses its value. The right approach is verification: check the citation, confirm the statistic. The Mata v. Avianca case happened because a lawyer skipped this step. Always verify claims that matter.
:::

## The Challenge

:::challenge{title="Hallucination Hunter" time="25 minutes"}
