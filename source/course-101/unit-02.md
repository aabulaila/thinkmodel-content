...
:::embed{type="video" title="Nobody Programmed This" src="/api/storage/public-objects/videos/2-Nobody_Programmed_This.mp4" youtubeId="-fTDvbjtZao"}
:::

:::socratic
question: You can't write step-by-step rules for recognizing a cat, but AI can do it easily. How do you think AI learns to do something that's so hard to describe in instructions?
hint: Think about how you learned to recognize faces as a baby — was it through rules, or through seeing lots of examples?
concept: Machine learning works by discovering patterns from examples, not from human-written rules.
:::
...
:::compare
before_label: Traditional Programming
before: A human writes every rule explicitly. The computer follows instructions exactly and never learns anything new. It can only do what someone told it to do. To change behavior, you rewrite the code.
after_label: Machine Learning
after: A human provides thousands of examples. The computer discovers its own rules from the data. It can find things humans never thought to look for. To change behavior, you change the training data.
:::
...
## AlphaGo's Move 37: The move no human would make
...
:::socratic
question: AlphaGo discovered a strategy that 3,000 years of human expertise missed. Does that mean the AI is "smarter" than humans at Go — or is something else going on?
hint: Think about what the AI actually did — it played millions of games against itself. Could any human do that? Is exploring more possibilities the same as being smarter?
concept: Machine learning discovers patterns through scale, not intelligence. AlphaGo wasn't smarter — it explored far more possibilities than any human could in a lifetime.
:::

But there's a flip side. Because nobody wrote the rules explicitly, nobody can fully explain *why* the system made a particular decision. This is the {{term:black box|A system where you can see what goes in and what comes out, but you can't see how it made its decision.}} problem, and it matters a lot when AI is making decisions about medical diagnoses, loan approvals, or criminal sentencing. We'll come back to this.
...
feedback_correct: That's the key distinction. Traditional programming is humans encoding rules explicitly. Machine learning is humans providing examples and the system finding its own patterns. This is why ML can discover things humans never thought to look for — like AlphaGo's Move 37.
feedback_incorrect: The answer is B. The core difference isn't about speed, accuracy, or internet access. It's about where the rules come from. In traditional programming, a human writes every rule. In machine learning, the human provides examples and the system discovers its own rules from those examples.
:::
...
:::embed{type="tool" title="Google Teachable Machine — Train your own AI" url="https://teachablemachine.withgoogle.com"}
:::
...
:::callout{type="tip"}
## Try this: Break your own model on purpose
After training, deliberately try to fool your model. Change the lighting dramatically, use a different background, have someone else test it, or partially obscure the object. Document each failure. Then add more diverse {{term:training examples|The labeled data you feed to a machine learning system so it can learn patterns. More variety in examples usually means a better model.}} and retrain. The improvement directly shows you why data diversity matters.
:::
...
Third, it shifts responsibility. When a traditional program fails, you can look at the code and find the bug. When a machine learning system fails, the problem might be in the data it was {{term:trained|The process of feeding a machine learning system examples so it can learn patterns. Training takes a lot of computing power and shapes everything the model can and can't do.}} on, the way the training was set up, or somewhere in the billions of {{term:parameters|The numbers inside an AI model that get adjusted during training, like its internal settings. They are counted in billions — roughly, more parameters means more capable and more memory-hungry.}} that no human can inspect one by one. This makes accountability harder — and that's a problem society is still figuring out.

:::socratic
question: When a traditional program makes a mistake, you can find the bug in the code. When a machine learning system makes a mistake, why is it so much harder to figure out what went wrong?
hint: Think about the difference between rules a human wrote (which you can read) and rules the system taught itself across billions of parameters (which nobody can fully inspect).
concept: Machine learning's "rules" are distributed across billions of learned parameters, making accountability and debugging fundamentally harder than in traditional software.
:::

:::quiz{id="u02-q2"}
question: When a machine learning system makes a biased decision (like favoring one group over another), the most likely cause is:
correct: C
...
feedback_correct: Exactly. ML systems learn from their training data. If that data reflects existing inequalities — more examples of some groups than others, historical patterns of discrimination — the system absorbs and reproduces those biases. Nobody "programmed" the bias, which is what makes it so hard to fix.
feedback_incorrect: The answer is C. Nobody deliberately codes bias into ML systems, and the AI doesn't have "opinions." The bias comes from the training data itself. If historical data reflects real-world inequalities, the system learns those patterns as if they're correct. This is harder to fix than a simple code bug because the "rules" are distributed across billions of learned parameters.
:::

## The Challenge

:::challenge{title="Teach the Machine, Break the Machine" time="30 minutes"}
...
:::embed{type="video" title="AlphaGo — Full Documentary (90 min)" id="WXuK6gekU1Y" link="true"}
:::
