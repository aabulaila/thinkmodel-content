...
:::embed{type="video" title="Why Context Engineering Wins" src="/api/storage/public-objects/videos/7-The_Briefing__Why_Context_Engineering_Wins.mp4" youtubeId="dyTHF2WrMYU"}
:::

:::socratic
question: When you ask AI a vague question and get a vague answer, is the AI being lazy — or is something else going on? What do you think determines the quality of AI output?
hint: Think about the intern analogy — if you give a vague brief, what kind of work do you get back?
concept: AI output quality is primarily determined by input quality — the more relevant context you provide, the less the AI has to guess.
:::
...
So what's the difference? "Prompt engineering" sounds like you're tweaking words to game the system. {{term:Context engineering|Giving AI everything it needs to do a great job: who you are, what you want, what good looks like, what to avoid, and any helpful background info.}} is about something much more substantial: **designing the complete information environment the AI needs to do its best work.**

:::analogy-target{id="p-context-intern"}
...
## How Anthropic engineers use context engineering
...
**Curate what goes in.** AI has a limited {{term:context window|The maximum amount of text an AI can "hold in mind" at once, like its working memory. Anything outside this limit is invisible to the AI.}} (working memory). Don't dump everything in; select what's relevant.
...
**Include examples.** Showing the AI what good output looks like — even one example — dramatically improves results. This is called {{term:few-shot prompting|Giving the AI one or more examples of what you want in your prompt so it can follow the pattern. Even one example makes a big difference.}}.
...
## Common misconception: Context engineering is about finding "magic prompts"
...
:::socratic
question: What's the difference between "finding the right words" and "providing the right information" when talking to AI? Why does this distinction matter?
hint: Think about magic prompt templates vs. actually briefing the AI with relevant details.
concept: Context engineering is about substance (information) not syntax (word choice). The right information always outperforms the right phrasing.
:::
...
:::quiz{id="u07-q1"}
question: What's the key difference between "prompt engineering" and "context engineering"?
correct: C
...
feedback_correct: Exactly. Prompt engineering implies tweaking words to game the system. Context engineering is about providing the right information: who you are, what you need, who it's for, what good looks like, and what to avoid. It's about substance, not syntax — and it produces dramatically better results because the AI has to guess less.
feedback_incorrect: The answer is C. These aren't two names for one thing, neither requires programming, and they aren't beginner and expert versions of the same move. The distinction is fundamental: prompt engineering focuses on how you phrase your request. Context engineering focuses on what information you include. A well-designed input with relevant context, examples, and constraints will outperform any cleverly worded prompt.
:::

:::socratic
question: If everyone has access to the same AI, why do some people get dramatically better results? What's the real differentiator?
hint: Think about context engineering — it's not about the tool, it's about how you use it.
concept: The quality of AI interaction is a skill — context engineering is the highest-leverage skill in working with AI.
:::
...
:::compare
before_label: Without context
before: "Write a cover letter for a marketing job." → Generic, formal, could apply to anyone applying for any marketing role. Uses clichéd phrases. Has no personality, no specificity, no sense of who the applicant is or what company they're applying to.
after_label: With full context
after: Identity + audience + tone + examples + constraints → Specific to this applicant, this company, this role. Conversational but professional. Opens with brand knowledge. Avoids clichés. Reads like it was written by someone who actually cares about sneakers. Same model, dramatically different output.
:::

:::callout{type="tip"}
## Try this: Build a context template for tasks you repeat
...
:::quiz{id="u07-q2"}
question: You ask AI to write an email and the output is generic and unhelpful. The most effective fix is to:
correct: B
...
feedback_correct: Context is the fix. The AI didn't produce bad output because it's bad at emails — it produced generic output because you gave it generic input. Adding who, what, for whom, tone, constraints, and examples gives the AI enough information to produce something specific and useful. This is context engineering in action.
feedback_incorrect: The answer is B. Switching models rarely fixes a context problem. Telling it to try harder doesn't give the AI more information to work with. And regenerating the same input just produces variations of the same generic answer. The fix is adding context: who you are, who it's for, the tone, the constraints, and ideally an example. More information in → better output out.
:::

## The Challenge

:::challenge{title="The Context Engineering Workout" time="30 minutes"}
